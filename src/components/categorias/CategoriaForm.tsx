...
export function CategoriaForm({ mode, categoria, returnTo = '/categorias' }: CategoriaFormProps) {
  ...
  const [tiposPlanes, setTiposPlanes] = useState<TipoPlanConfig[]>(() => {
    ...
    if (categoria.tiposPlanes && categoria.tiposPlanes.length > 0) return categoria.tiposPlanes;
    
    // Auto-migrate legacy plans into virtual types so they show up in the UI
    const legacyTipos: TipoPlanConfig[] = [];
    const hasCuentaCompleta = categoria.planes?.some(p => p.tipoPlan === 'cuenta_completa');
    const hasPerfiles = categoria.planes?.some(p => p.tipoPlan === 'perfiles');
    
    if (hasCuentaCompleta) legacyTipos.push({ id: 'cuenta_completa', nombre: 'Cuenta Completa' });
  ...
  const [tipoSeleccionadoId, setTipoSeleccionadoId] = useState<string | null>(() => {
    ...
    if (categoria.tiposPlanes && categoria.tiposPlanes.length > 0) return categoria.tiposPlanes[0].id;
  ...
  const hasChanges = useMemo(() => {
    ...
    for (let i = 0; i < tiposPlanes.length; i++) {
      ...
      if (!orig || curr.id !== orig.id || curr.nombre !== orig.nombre) return true;
    }
    ...
    return false;
  }, [mode, categoria, nombreValue, tipoValue, tipoCategoriaValue, notasValue, planes, tiposPlanes]);

  useEffect(() => {
    if (nombreValue && nombreValue.length >= 2 && errors.nombre) clearErrors('nombre');
  ...
  useEffect(() => {
    if (tipoCategoriaValue && errors.tipoCategoria) clearErrors('tipoCategoria');
  ...
  const handleAgregarTipo = () => {
    ...
    if (tiposPlanes.some(t => t.nombre.toLowerCase() === trimmed.toLowerCase())) {
      setTipoNombreError('Ya existe un tipo con ese nombre');
      return;
    }
    const nuevoTipo: TipoPlanConfig = { id: `tipo-${Date.now()}`, nombre: trimmed };
  ...
  const handleGuardarEdicion = (e?: React.MouseEvent | React.KeyboardEvent) => {
    ...
    if (tiposPlanes.some(t => t.id !== editandoTipoId && t.nombre.toLowerCase() === trimmed.toLowerCase())) {
      setEditTipoError('Ya existe un tipo con ese nombre');
      return;
    }

    setTiposPlanes(prev => prev.map(t => t.id === editandoTipoId ? { ...t, nombre: trimmed } : t));
    setEditandoTipoId(null);
  };

  const handleCancelarEdicion = (e?: React.MouseEvent | React.KeyboardEvent) => {
    if (e) e.stopPropagation();
    setEditandoTipoId(null);
  };
  ...
  const actualizarPlan = (id: string, campo: keyof Plan, valor: string | number) => {
    setPlanes(prev => prev.map(plan => plan.id === id ? { ...plan, [campo]: valor } : plan));
  };

  const planesDeTipoActual = useMemo(
    () => tipoSeleccionadoId ? planes.filter(p => p.tipoPlan === tipoSeleccionadoId) : [],
  ...
  const getTipoCategoriaLabel = (tipo: string) => {
    switch (tipo) {
      ...
      default: return 'Seleccionar tipo';
    }
  };
  ...
  const handleTabChange = async (value: string) => {
    if (value === 'planes' && !isGeneralTabComplete) {
      ...
      if (isValid) { setIsGeneralTabComplete(true); setActiveTab(value); }
    } else {
      setActiveTab(value);
    }
  };
  ...
  const handleNext = async () => {
    const isValid = await trigger(['nombre', 'tipo', 'tipoCategoria']);
    if (isValid) { setIsGeneralTabComplete(true); setActiveTab('planes'); }
  };
        ...
        <TabsContent value="general" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="w-[var(--radix-dropdown-menu-trigger-width)]">
                  <DropdownMenuItem onClick={() => setValue('tipo', 'cliente')}>Cliente</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setValue('tipo', 'revendedor')}>Revendedor</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            ...
            <div className="space-y-2">
              ...
              {errors.tipoCategoria && <p className="text-sm text-red-500">{errors.tipoCategoria.message}</p>}
            </div>
          </div>

          <div className="space-y-2">
            <Label htmlFor="notas">Notas</Label>
            <Textarea id="notas" {...register('notas')} placeholder="Añade notas sobre la categoría..." rows={6} />
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="lg:w-[260px] shrink-0 space-y-3">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">Crea tus propias categorías de planes</p>
              </div>
              ...
              <div className="flex flex-col gap-2">
                {tiposPlanes.map((tipo) => {
                  ...
                  if (editandoTipoId === tipo.id) {
                    return (
                      <div key={tipo.id} className="p-2 border-2 border-primary/50 rounded-lg bg-background" onClick={e => e.stopPropagation()}>
                        <Input
                          autoFocus
                          value={editTipoNombre}
                          onChange={(e) => { setEditTipoNombre(e.target.value); setEditTipoError(''); }}
                          className="text-sm h-8 mb-2"
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') { e.preventDefault(); handleGuardarEdicion(e); }
                            if (e.key === 'Escape') { handleCancelarEdicion(e); }
                          }}
                        />
                        {editTipoError && <p className="text-xs text-red-500 mb-2">{editTipoError}</p>}
                        <div className="flex gap-2 justify-end">
                          <button type="button" onClick={handleCancelarEdicion} className="h-6 w-6 flex items-center justify-center rounded text-muted-foreground hover:bg-muted">
                            <X className="h-3.5 w-3.5" />
                          </button>
                          <button type="button" onClick={handleGuardarEdicion} className="h-6 w-6 flex items-center justify-center rounded text-primary hover:bg-primary/10">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                          type="button"
                          onClick={(e) => { e.stopPropagation(); handleEliminarTipo(tipo.id); }}
                ...
                {showNuevoTipoInput ? (
                  <div className="space-y-1.5 pt-1">
                    <Input
                      ...
                      onChange={(e) => { setNuevoTipoNombre(e.target.value); setTipoNombreError(''); }}
                      placeholder="Ej: Pantalla Completa"
                      className="text-sm h-9"
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') { e.preventDefault(); handleAgregarTipo(); }
                        if (e.key === 'Escape') { setShowNuevoTipoInput(false); setNuevoTipoNombre(''); setTipoNombreError(''); }
                      }}
                    />
                    {tipoNombreError && <p className="text-xs text-red-500">{tipoNombreError}</p>}
                    <div className="flex gap-2">
                      <Button type="button" size="sm" className="flex-1 h-8 text-xs" onClick={handleAgregarTipo}>
                        Agregar
                      </Button>
                      <Button
                        type="button" size="sm" variant="outline" className="flex-1 h-8 text-xs"
                        onClick={() => { setShowNuevoTipoInput(false); setNuevoTipoNombre(''); setTipoNombreError(''); }}
                      >
                        Cancelar
                      </Button>
                    </div>
            ...
            <div className="flex-1 space-y-4">
              {!tipoActual ? (
                <div className="flex flex-col items-center justify-center py-16 text-center border-2 border-dashed rounded-lg">
                  ...
                  <p className="text-sm font-medium text-muted-foreground">Sin tipo seleccionado</p>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="text-base font-semibold">Planes — {tipoActual.nombre}</h3>
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {planesDeTipoActual.length} plan{planesDeTipoActual.length !== 1 ? 'es' : ''} para este tipo
                      </p>
                    </div>
                    <Button type="button" onClick={() => agregarPlan(tipoActual.id)} size="sm">
                      ...
                      {planesDeTipoActual.map((plan, index) => (
                        <div key={plan.id} className="p-4 border rounded-lg bg-card space-y-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="space-y-2">
                              <Label htmlFor={`plan-nombre-${plan.id}`}>Nombre del Plan</Label>
                              <Input
                            ...
                            <div className="space-y-2">
                              ...
                              <Input
                                ...
                                onChange={(e) => actualizarPlan(plan.id, 'precio', parseFloat(e.target.value) || 0)}
                                placeholder="$0.00"
                            ...
                            <div className="space-y-2">
                              <Label htmlFor={`plan-ciclo-${plan.id}`}>Período de Tiempo</Label>
                              <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                  <Button variant="outline" className="w-full justify-between" type="button">
                                ...
                                <DropdownMenuContent align="start" className="w-[var(--radix-dropdown-menu-trigger-width)]">
                                  <DropdownMenuItem onClick={() => actualizarPlan(plan.id, 'cicloPago', 'mensual')}>Mensual</DropdownMenuItem>
                                  <DropdownMenuItem onClick={() => actualizarPlan(plan.id, 'cicloPago', 'trimestral')}>Trimestral</DropdownMenuItem>
                                  <DropdownMenuItem onClick={() => actualizarPlan(plan.id, 'cicloPago', 'semestral')}>Semestral</DropdownMenuItem>
                                  <DropdownMenuItem onClick={() => actualizarPlan(plan.id, 'cicloPago', 'anual')}>Anual</DropdownMenuItem>
                                </DropdownMenuContent>
                              </DropdownMenu>
                            </div>
