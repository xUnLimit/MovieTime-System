...
export function CategoriaEditForm({ categoria }: CategoriaEditFormProps) {
  const router = useRouter();
  const { updateCategoria } = useCategoriasStore();
  const [activeTab, setActiveTab] = useState('general');
  const [isGeneralTabComplete, setIsGeneralTabComplete] = useState(true);
  const [planes, setPlanes] = useState<Plan[]>(categoria.planes || []);
  ...
  const hasChanges = useMemo(() => {
    ...
    return false;
  }, [nombreValue, tipoValue, tipoCategoriaValue, notasValue, planes, categoria]);
