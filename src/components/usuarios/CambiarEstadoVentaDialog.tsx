      ...
      <DialogContent className="sm:max-w-[420px] p-0 overflow-hidden gap-0">
        ...
        <div className="px-6 pt-6 pb-4 bg-muted/30">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-base">
              <div className={`flex items-center justify-center w-7 h-7 rounded-full ${esActivar ? 'bg-green-100 dark:bg-green-950/40' : 'bg-red-100 dark:bg-red-950/40'}`}>
                {esActivar
          ...
          <div className="mt-3 space-y-1 text-sm">
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground w-20 shrink-0">Categoría</span>
              <span className="font-medium">{venta.categoriaNombre}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground w-20 shrink-0">Servicio</span>
              <span className="font-medium">{venta.servicioNombre}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground w-20 shrink-0">Acción</span>
              <Badge
                variant="outline"
                className={esActivar
        ...
        <div className="px-6 py-4">
          <div className="space-y-3">
            <p className="text-sm font-medium text-muted-foreground">¿Qué deseas {esActivar ? 'activar' : 'inactivar'}?</p>
            <RadioGroup
              value={alcance}
            ...
            >
              ...
              >
                <RadioGroupItem value="venta" id="opt-solo-venta" className="mt-0.5" />
                <div className="space-y-0.5">
                  <div className="flex items-center gap-1.5">
                    <ShoppingCart className={`h-3.5 w-3.5 ${esActivar ? 'text-green-600' : 'text-red-600'}`} />
              ...
              >
                <RadioGroupItem value="venta_y_servicio" id="opt-venta-servicio" className="mt-0.5" />
                <div className="space-y-0.5">
                  <div className="flex items-center gap-1.5">
                    <Monitor className={`h-3.5 w-3.5 ${esActivar ? 'text-green-600' : 'text-red-600'}`} />
                    <span className="text-sm font-medium">Venta y servicio</span>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {esActivar
        ...
        <DialogFooter className="px-6 pb-5 pt-2 flex gap-2">
          ...
          >
            {isSubmitting ? 'Procesando...' : esActivar ? 'Activar' : 'Inactivar'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
