...
type RenovacionFormData = z.infer<typeof renovacionSchema>;

interface RenovarServicioDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  servicio: Servicio;
  metodosPago: MetodoPago[];
  onConfirm: (data: RenovacionFormData) => void;
}

export function RenovarServicioDialog({
  open,
  onOpenChange,
  servicio,
  metodosPago,
  onConfirm,
}: RenovarServicioDialogProps) {
  ...
  // Auto-calcular fecha de vencimiento cuando el usuario selecciona el ciclo de facturación (desde fecha de inicio)
  useEffect(() => {
  ...
  const onSubmit = async (data: RenovacionFormData) => {
    onConfirm(data);
    onOpenChange(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Renovar Servicio: {servicio.nombre}</DialogTitle>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-4">
          {/* Ciclo de facturación | Método de pago | Costo */}
